<?php
	...
	if (!empty($_POST['id_mahasiswa']) && !empty($_POST['mahasiswa_nama']) && !empty($_POST['mahasiswa_nohp']) && !empty($_POST['Mahasiswa_alamat'])) {
		...
		$query = "UPDATE mahasiswa set mahasiswa_nama = '$mahasiswa_nama' , mahasiswa_nohp = '$mahasiswa_nohp', Mahasiswa_alamat = '$Mahasiswa_alamat', avatar = '$avatar' WHERE id_mahasiswa = '$id_mahasiswa' " ;
